...
async def manage_tasks_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        text="Choose action", reply_markup=task_decision_keyboard()
    )
...
async def add_task_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        text=f"You've set the task!", reply_markup=ReplyKeyboardRemove()
    )
    await message.answer(
...
async def drop_task_handler(message: Message, state: FSMContext) -> None:
    ...
    if tasks == {}:
        await message.answer(
            text="You've not have any tasks yet", reply_markup=menu_keyboard()
        )
    else:
        text = ""
        for task, value in tasks.items():
            for user, sub_value in value.items():
                if user == message.from_user.id:
                    text += (
                        f"Task {task}:\n"
                        f"Get info about changes in {sub_value['coin_name'].upper()} for period "
                        f"{sub_value['time_period']} at {sub_value['repeat_time']} every day\n"
                        f"---\n"
                    )
...
async def drop_task_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        text='Type task to drop (ex: 0 or 1 - U can see it by command "Show all tasks")'
    )
...
async def drop_task_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        text=f"You dropped task {message.text}!", reply_markup=ReplyKeyboardRemove()
    )
    await message.answer(
        text=f"What do you like to do next?", reply_markup=menu_keyboard()
    )
